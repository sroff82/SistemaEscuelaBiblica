                                    ...
                                    {isExpanded && (
                                        <div className="p-5 pt-4 animate-in slide-in-from-top-2 duration-200 border-t border-slate-100">
                                            <div className="mb-5 bg-white p-3 rounded-xl border border-slate-100 shadow-sm">
                                                ...
                                                <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden"><div className="bg-indigo-500 h-1.5 rounded-full transition-all duration-1000" style={{width: `${prog.porc}%`}}></div></div>
                                            </div>
                                            <div className="flex space-x-2">
                                                                ...
                                                                return (
                                                                    <div key={grupo.id} className="bg-slate-50 rounded-xl border border-slate-200 overflow-hidden shadow-sm transition-all duration-300">
                                                                        <button onClick={() => { setMesClasesExp(isExpMes ? null : `${campo}-${grupo.id}`); setSemanaClasesExp(null); }} className="w-full p-3 flex justify-between items-center bg-white hover:bg-slate-50 transition-colors">
                                                                            <div className="text-left"><p className="font-bold text-slate-700 text-xs capitalize">{grupo.mesLabel}</p></div>
                                                                            <div className="flex items-center space-x-2 text-[9px] font-black">
                                                                                    ...
                                                                                    return (
                                                                                        <div key={sem.id} className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                                                                                            <button onClick={() => setSemanaClasesExp(isSemExp ? null : `${campo}-${grupo.id}-${sem.id}`)} className="w-full p-2.5 flex justify-between items-center hover:bg-slate-50 transition-colors">
                                                                                                <div className="text-left"><p className="font-bold text-slate-600 text-[10px]">{sem.label}</p></div>
                                                                                                <div className="flex items-center space-x-2 text-[9px] font-black">
                    ...
                    {subVistaAdminLogistica === 'bodega' && (
                        <div className="animate-in slide-in-from-left duration-200 px-1 mt-2">
                            <div className="bg-indigo-600 p-6 rounded-[32px] text-white shadow-xl shadow-indigo-200 relative overflow-hidden mb-5 mt-2">
                                ...
                                <div className="relative z-10">
                                    <p className="text-xs font-bold opacity-80 tracking-widest mb-1">Stock Físico en Bodega</p>
                            ...
                            <div className="bg-slate-50 rounded-3xl p-5 mb-5 border border-slate-200 shadow-sm">
                                ...
                                <button onClick={() => onCerrarJornada(rutasParaArchivar)} className="w-full py-3 bg-white text-rose-500 border border-rose-200 font-bold rounded-xl shadow-sm text-[11px] uppercase tracking-widest active:scale-95 transition-all"><i className="fas fa-flag-checkered mr-2"></i> Cerrar Jornada</button>
                            </div>
                            <form onSubmit={(e) => { e.preventDefault(); onActualizarInventario(Number(e.target.nuevoStock.value)); e.target.reset(); }} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm">
                                ...
                                <div className="flex space-x-3">
                                    <input type="number" name="nuevoStock" required placeholder="Ej: 100 o -20" className="w-2/3 p-4 bg-slate-50 rounded-2xl border border-slate-200 text-lg font-black text-center" />
                                    <button type="submit" className="w-1/3 bg-indigo-600 text-white font-black rounded-2xl shadow-md active:scale-95 transition-all">Ajustar</button>
                                </div>
                            </form>
                        </div>
                    )}
                    ...
                    {subVistaAdminLogistica === 'misiones' && (
                        <>
                            <form onSubmit={submitMision} className="bg-amber-50 p-6 rounded-[32px] border border-amber-100 shadow-sm mx-1 mt-2">
                                <h3 className="font-bold text-amber-800 text-sm mb-4 flex items-center"><i className="fas fa-map-marked-alt mr-2"></i> Asignar Ruta</h3>
                                <div className="space-y-4">
                                    <div className="grid grid-cols-2 gap-2 max-h-32 overflow-y-auto p-3 bg-white rounded-2xl border border-amber-100 shadow-inner">
                                        {camposFijos.map(c => (
                                            <label key={c} className="flex items-center space-x-2 text-xs font-bold text-slate-600">
                                                <input type="checkbox" checked={camposRuta.includes(c)} onChange={() => toggleCampoRuta(c)} />
                                                <span>{c}</span>
                                            </label>
                                        ))}
                                    </div>
                                    <div className="flex space-x-3">
                                        <input type="number" name="cantidad" required placeholder="Cant." className="w-1/2 p-4 bg-white rounded-2xl border border-amber-100 text-center" />
                                        <select name="grupo" required className="w-1/2 p-4 bg-white rounded-2xl border border-amber-100">
                                            <option value="">¿Grupo?</option>
                                            {[1, 2, 3, 4, 5, 6, 7, 8].map(n => (<option key={n} value={`Grupo ${n}`}>Grupo {n}</option>))}
                                        </select>
                                    </div>
                                    <button type="submit" className="w-full py-4 bg-amber-500 text-white font-black rounded-2xl">Enviar Ruta</button>
                                </div>
                            </form>
                            ...
                            <div className="mt-6 px-1">
                                {entregasPendientes.map(e => {
                                    ...
                                    return (
                                        <div key={e.id} className="bg-white p-4 rounded-2xl border-l-4 border-l-amber-400 shadow-sm mb-3">
                                            <div className="flex justify-between">
                                                <p className="font-black text-slate-800">{e.grupo}</p>
                                                <button onClick={() => onBorrarEntrega(e.id)} className="text-rose-500"><i className="fas fa-trash-alt"></i></button>
                                            </div>
                                            <p className="text-xs text-slate-500 font-bold mt-1">Carga: {e.cantidad} | Avance: {totalEntregado}</p>
                                        </div>
                                    );
                                })}
                            </div>
                        ...
                        <div className="px-1 mt-2 space-y-3">
                            {personalLogistica.map(p => (
                                <div key={p.id} className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center justify-between shadow-sm">
                                    ...
                                    <select value={p.grupo || ''} onChange={(e) => onAssignGroup(p.id, e.target.value)} className="p-2 rounded-xl border font-bold text-xs">
        ...
        contenidoAdmin = (
            <div className="space-y-4 animate-in slide-in-from-right duration-300 h-full flex flex-col pt-2">
                <div className="flex justify-between items-center px-2 mb-2">
                    ...
                    <button onClick={onToggleModal} className="w-10 h-10 bg-indigo-600 text-white rounded-xl flex items-center justify-center"><i className="fas fa-plus"></i></button>
                </div>
                <div className="flex items-center bg-white rounded-2xl px-5 py-4 shadow-sm border border-slate-100 mx-1"><i className="fas fa-search text-slate-300 mr-3"></i><input type="text" placeholder="Buscar..." className="bg-transparent w-full outline-none text-sm font-bold text-slate-700" value={busqueda} onChange={(e) => setBusqueda(e.target.value)} /></div>
                <div className="flex-1 overflow-y-auto space-y-3 pb-24 pr-1 mt-2 px-1">
                    {rolesConGente.map(rolGrp => (
                        <div key={rolGrp} className="bg-white rounded-[24px] border border-slate-100 shadow-sm overflow-hidden">
                            <button onClick={() => setRolExpandido(rolExpandido === rolGrp ? null : rolGrp)} className="w-full p-4 flex justify-between items-center">
                                <span className="font-black text-slate-700 text-sm uppercase">{rolGrp}</span>
                                <i className="fas fa-chevron-down text-slate-300"></i>
                            </button>
                            {rolExpandido === rolGrp && (
                                <div className="p-4 bg-slate-50 space-y-2">
                                    {gruposPersonal[rolGrp].map(m => (
                                        <div key={m.id} className="bg-white p-3 rounded-2xl border border-slate-100 flex justify-between items-center">
                                            <div>
                                                <p className="font-bold text-slate-700 text-xs">{m.nombre}</p>
                                                <p className="text-[9px] text-slate-400 uppercase mt-0.5">{m.campo || 'Global'}</p>
                                            </div>
                                            <div className="flex space-x-2">
                                                <button onClick={() => onDelete(m)} className="text-rose-500"><i className="fas fa-trash"></i></button>
                                                <button onClick={() => onEdit(m)} className="text-indigo-500"><i className="fas fa-edit"></i></button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
    ...
    return (
        <>
            ...
            <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/90 backdrop-blur-md border-t border-slate-100 flex justify-around items-center p-2 z-50 pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
                ...
                <NavButton id="logistica" icon="fa-truck" label="Logística" />
